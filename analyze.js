    ...
    export default async function handler(req, res) {
      try {
        ...
        const result = { symbol, aiResults, decision };
    ...
    async function fetchNews(symbol){
      try{
        const r = await fetch(`https://cryptopanic.com/api/v1/posts/?auth_token=demo&currencies=${symbol}`);
        const d = await r.json();
        return d.results.slice(0,5).map(n=>({
          sentiment: n.votes?.positive > n.votes?.negative ? "positive":"negative"
        }));
      }catch{
        return [];
      }
    ...
    async function runAI(summary){
      ...
      const results = await Promise.allSettled(MODELS.map(async (model)=>{
        try{
          ...
          if(!r.ok) return "HOLD";
          const d = await r.json();
          const t = d?.choices?.[0]?.message?.content?.toUpperCase() || "";
